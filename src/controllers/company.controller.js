...
const updateMyGalleryItem = catchAsync(async (req, res) => {
    const item = await companyService.updateMyGalleryItem(req.user.id, req.params.galleryItemId, req.body);
...
const deleteMyContribution = catchAsync(async (req, res) => {
    const contribution = await companyService.deleteMyContribution(req.user.id, req.params.contributionId);
