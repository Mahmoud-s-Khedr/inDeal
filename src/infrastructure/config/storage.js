...
if (!hasCredentials) {
  log.warn(
    'S3 client initialized without explicit credentials. Ensure IAM roles are available in runtime environment.'
  );
} else {
  log.info(
    { bucket: config.storage.bucket, endpoint: config.storage.endpoint?.substring(0, 30) + '...' },
...
const deleteObject = async (key) => {
  ...
  try {
    ...
    log.info({ operation: 'delete', key, durationMs: duration }, 'R2 delete completed');

    return result;
  } catch (err) {
    const duration = Date.now() - start;
    log.error({ err, operation: 'delete', key, durationMs: duration }, 'R2 delete failed');
    throw err;
  }
};
