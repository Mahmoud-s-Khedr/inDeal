...
- Without cookies, there is **no way** for the backend to “silently” update the client’s token with *zero* client-side handling.

What we *can* still achieve (typical compromise):
...
If you truly want *zero* token updates on the client, then rotation must be disabled and the same token remains until `exp` (this reduces the security value of short access windows).
...
We store a *server-side session record* keyed by `jti` (the JWT id). This makes the JWT **stateful**.
