...
export async function runContributionsFlow(credentials = null) {
    return runFlow(
        ...
        async (results) => {
            ...
            results.push(await step('Upload first media file (image)', async () => {
            ...
            results.push(await step('Upload second media file (document)', async () => {
            ...
            results.push(await step('Create contribution', async () => {
            ...
            let createdMedia;
            results.push(await step('Add media item to contribution', async () => {
            ...
            let mediaList;
            results.push(await step('List contribution media', async () => {
                ...
                mediaList.data?.forEach((item, i) => {
                    console.log(chalk.gray(`      ${i + 1}. [${item.mediaType}] ${item.caption || 'No caption'} (order: ${item.sortOrder})`));
                });
                return mediaList;
            }));
            ...
            if (mediaList.data?.length > 1) {
                results.push(await step('Reorder media items', async () => {
            ...
            results.push(await step('Update contribution', async () => {
                const updateData = {
            ...
            results.push(await step('Update media item', async () => {
                const updateData = {
                    caption: `Updated caption - ${Date.now()}`,
                };
                const updated = await apiClient.updateContributionMedia(contributionId, mediaId, updateData);
            ...
            results.push(await step('Delete media item', async () => {
            ...
            results.push(await step('List contributions', async () => {
            ...
            results.push(await step('Delete contribution', async () => {
            ...
            results.push(await step('Verify deletion', async () => {
                const contributions = await apiClient.getMyContributions();
