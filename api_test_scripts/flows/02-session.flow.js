...
export async function runSessionFlow(credentials = null) {
  return runFlow(
    ...
    async (results) => {
      ...
      let userProfile;
      results.push(
        await step('Get user profile (/users/me)', async () => {
          userProfile = await apiClient.getMe();
          console.log(
            chalk.gray(`    Name: ${userProfile.data?.firstName} ${userProfile.data?.lastName}`)
          );
      ...
      let companyProfile;
      results.push(
        await step('Get company profile (/companies/me)', async () => {
      ...
      results.push(
        await step('Verify token invalidated', async () => {
          try {
