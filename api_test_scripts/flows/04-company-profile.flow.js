...
export async function runCompanyProfileFlow(credentials = null) {
    return runFlow(
        ...
        async (results) => {
            ...
            let currentProfile;
            results.push(await step('Get current company profile', async () => {
            ...
            results.push(await step('Update company profile', async () => {
            ...
            results.push(await step('Verify profile changes', async () => {
            ...
            if (currentStatus !== 'active') {
                results.push(await step('Submit for review', async () => {
                    const result = await apiClient.resendForReview();
                    console.log(chalk.gray('    Company submitted for admin review'));
                    return result;
                }));
            } else {
                results.push(await step('Skip review submission (already active)', async () => {
