...
export async function runPasswordResetFlow(testEmail = null, testOtp = null) {
  return runFlow('Password Reset', 'Request OTP, verify, and reset password', async (results) => {
    ...
    // Step 1: Request OTP
    results.push(
      await step('Request password reset OTP', async () => {
        const result = await apiClient.forgotPassword(email);
        console.log(chalk.yellow('    ⚠ OTP sent to email'));
        console.log(
          chalk.gray('    Check email and update testTokens.passwordResetOtp in test-data.json')
        );
        return result;
      })
    );
    ...
    if (!otp) {
      results.push(
        await step('Waiting for OTP (manual step)', async () => {
