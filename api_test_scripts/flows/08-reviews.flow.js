...
export async function runReviewsFlow(credentials = null, targetCompanyId = null) {
    return runFlow(
        'Company Reviews',
        'View companies and submit reviews',
        async (results) => {
            ...
            let myCompanyId;
            results.push(await step('Get own company (to avoid self-review)', async () => {
            ...
            if (!companyToReview) {
                results.push(await step('Find company to review', async () => {
                        ...
                        if (testId !== myCompanyId) {
                            try {
                                ...
                                if (company.data) {
                                    ...
                                    console.log(chalk.gray(`    Found company: ${company.data?.name || 'Unknown'} (ID: ${testId})`));
                                    return company;
                                }
                            } catch {
            ...
            results.push(await step('View target company profile', async () => {
            ...
            results.push(await step('View company gallery', async () => {
            ...
            results.push(await step('View existing reviews', async () => {
            ...
            results.push(await step('Submit review', async () => {
            ...
            results.push(await step('Verify review was added', async () => {
                const reviews = await apiClient.getCompanyReviews(companyToReview);
